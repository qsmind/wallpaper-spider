...
class producer(threading.Thread):
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    self.get_pic_list(html, pic_list)  # 获取套图      
                    # print(pic_list)
                    for link in pic_list:  # 爬取单个页面套图
                        # j += 1  # 已获取的套图数量
                        print(type(link))
                        print(link)
                        html = self.get_html(link)  # 获取网页
                        src=[]
                        self.get_pic_link(html,src)  # 获取图片链接
                        # 将图片链接存入队列
                        for ref in src:
                            response_q.put(ref)  
                    time.sleep(1) 
    ...
    def get_pic_list(self,html, pic_list):
        try:
            ...
            # print(soup)
            wrapper = soup.find('div','wrapper')
            for href in wrapper.find_all('a'):
                pic_list.append(href.attrs["href"])
            # pic_list.append(wrapper.find('a').get('href'))
            ...
            #     pic_list.append(link.find('a').get('href'))
                # temp = []
                # print(pic_list)
        except:
            # print('获取套图失败！')
